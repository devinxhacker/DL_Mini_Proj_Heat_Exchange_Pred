...
def restore_pilstm(artifact_path: Path, weights_path: Path) -> dict | None:
    ...
    try:
        ...
        for hard_mode in (True, False):
            try:
                pilstm = PhysicsInformedLSTM(
                    ...
                    residual_learning=bool(artifact.get("residual_learning", False)),
                    use_bidirectional=bool(artifact.get("use_bidirectional", False)),
                )
        ...
        if pilstm.residual_learning:
            pilstm.scaler_residual.mean_ = artifact["scaler_y_mean"]
            pilstm.scaler_residual.scale_ = artifact["scaler_y_scale"]
        else:
...
def predict_pilstm(
    ...
    try:
        ...
        hot_pred = float(
            pilstm_artifact.get("hot_calibration_slope", 1.0) * hot_pred
            + pilstm_artifact.get("hot_calibration_intercept", 0.0)
        )
